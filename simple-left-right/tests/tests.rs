...
mod tests {
    ...
    fn send_writer() {
        ...
        std::thread::spawn(move || {
            let mut writer = rcv.recv().unwrap();
    ...
    fn write_guard_drop() {
        ...
        assert_eq!(*write_lock.as_ref(), 0);
        write_lock.apply_op(CounterAddOp(2));
        assert_eq!(*write_lock.as_ref(), 2);
        // drop(write_lock);
        let write_lock = writer.lock();
        assert_eq!(*write_lock.as_ref(), 2);
        write_lock.swap();
        assert_eq!(*reader.lock(), 2);
        assert_eq!(*writer.lock().as_ref(), 2);
    }
    ...
    fn write_try_lock() {
        ...
        assert_eq!(*writer.try_lock().unwrap().as_ref(), 1);
    }
    ...
    #[test]
    fn reader_rebuild() {
        let mut writer: Writer<i32, CounterAddOp> = Writer::new_from_default();
        let reader = writer.build_reader().unwrap();
        drop(reader);
        let _reader = writer.build_reader().unwrap();
    }
    ...
    #[test]
    fn default_init() {
        let writer: Writer<i32, CounterAddOp> = Writer::new_from_default();
        assert_eq!(*writer.as_ref(), i32::default());
    }
